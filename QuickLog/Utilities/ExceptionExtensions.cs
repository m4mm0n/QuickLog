...
public static class ExceptionExtensions
{
    ...
    private static class FrameFormatter
    {
        ...
        public static bool IsInfrastructure(DemystifiedFrame f)
        {
            ...
            if (dt == typeof(ExecutionContext)) return true;
            if (dt == typeof(ThreadPool)) return true;
        ...
        private static string FormatTypeName(Type t)
        {
            ...
            // Nested type?
            return t.IsNested && t.DeclaringType != null ? FormatTypeName(t.DeclaringType) + "." + t.Name :
                // Full name, but avoid + for nested and use dots
                !string.IsNullOrEmpty(t.FullName) ? t.FullName!.Replace('+', '.') : t.Name;
        }
